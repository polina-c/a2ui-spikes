...
describe('Catalog.fromSchema & schema_loader', () => {
    const basicCatalogPath = resolve(process.cwd(), '../../specification/v0_9_1/catalogs/basic/catalog.json');
    const basicCatalogJson = JSON.parse(readFileSync(basicCatalogPath, 'utf-8'));
    it('throws error when catalogId is missing', () => {
        const invalidJson = {
            components: {},
        };
        assert.throws(() => Catalog.fromSchema(invalidJson), /Catalog ID must be specified/);
    });
    it('loads basic catalog successfully and dynamically resolves weight and accessibility', () => {
        ...
        assert.strictEqual(catalog.id, 'https://a2ui.org/specification/v0_9/catalogs/basic/catalog.json');
        ...
        const textShape = textComp.schema.shape;
        ...
        assert.ok(rowComp);
        const rowShape = rowComp.schema.shape;
        ...
        assert.ok(btnComp);
        const btnShape = btnComp.schema.shape;
    ...
    it('dynamically resolves custom local $defs in allOf and property references', () => {
        ...
        assert.ok(widget);
        const shape = widget.schema.shape;
...
//# sourceMappingURL=schema_loader.test.js.map