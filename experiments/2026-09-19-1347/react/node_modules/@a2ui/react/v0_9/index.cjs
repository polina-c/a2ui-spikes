...
var NodeSurfaceContext = React3.createContext(
  null
);
var LoadingPlaceholder = ({ componentId }) => /* @__PURE__ */ jsxRuntime.jsxs("div", { style: { color: "gray", padding: "4px" }, children: [
  "[Loading ",
  componentId,
  "...]"
] });
...
var UnresolvedChildReference = ({ surface, id, requestedPath, detail }) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { style: { color: "red" }, children: message });
};
function useSignalValue(signal) {
  const subscribe = React3.useCallback(
    (onChange) => v0_9.effect(() => {
      v0_9.getValue(signal);
      onChange();
    }),
    [signal]
  );
  const getSnapshot = React3.useCallback(() => v0_9.peekValue(signal), [signal]);
  return React3.useSyncExternalStore(subscribe, getSnapshot);
}
...
function toViewValue(parent, value, index) {
  ...
  if (value instanceof v0_9.ResolvedBinding) {
...
function toViewProps(parent, props, index) {
  ...
  for (const [key, inner] of Object.entries(props)) {
    if (inner instanceof v0_9.ResolvedBinding) {
      result[key] = toViewValue(parent, inner.value, index);
      result[`set${key.charAt(0).toUpperCase()}${key.slice(1)}`] = v0_9.isWritable(inner) ? inner.set : () => {
      };
    } else {
...
function useNodeView(node, buildChild) {
  const surface = React3.useContext(NodeSurfaceContext);
  const resolved = useSignalValue(node.props);
  const { viewProps, childIndex } = React3.useMemo(() => {
    const index = newChildIndex();
    return { viewProps: toViewProps(node, resolved, index), childIndex: index };
  }, [node, resolved]);
  const context = React3.useMemo(
    () => surface && surface.componentsModel.get(node.componentId) ? new v0_9.ComponentContext(surface, node.componentId, node.dataPath) : void 0,
    [surface, node]
  );
  const resolveThrough = React3.useCallback(
    (map, id, basePath) => {
...
var RenderFallback = ({ node, impl, buildChild }) => {
  ...
  if (!context) {
    return /* @__PURE__ */ jsxRuntime.jsx(LoadingPlaceholder, { componentId: node.componentId });
  }
  return /* @__PURE__ */ jsxRuntime.jsx(Render, { context, buildChild: rawBuildChild });
};
var NodeView = React3.memo(
  ({
    surface,
    node
  }) => {
    const buildChild = React3.useCallback(
      (child, basePath) => {
        if (v0_9.isComponentNode(child)) {
          return /* @__PURE__ */ jsxRuntime.jsx(NodeView, { surface, node: child }, child.instanceId);
        }
        const requested = basePath ?? node.dataPath;
    ...
    if (!View) {
      return /* @__PURE__ */ jsxRuntime.jsx(RenderFallback, { node, impl, buildChild });
    }
    return /* @__PURE__ */ jsxRuntime.jsx(View, { node, buildChild });
  }
);
NodeView.displayName = "NodeView";
var A2uiSurface = ({ surface }) => {
  const box = React3.useMemo(
    () => ({ resolver: void 0 }),
  ...
  const subscribe = React3.useCallback(
    (onChange) => {
      const resolver = new v0_9.NodeResolver(surface, surface.catalog);
      box.resolver = resolver;
      const stopEffect = v0_9.effect(() => {
        v0_9.getValue(resolver.rootNode);
        onChange();
      });
      return () => {
        stopEffect();
  ...
  const getSnapshot = React3.useCallback(
    () => box.resolver ? v0_9.peekValue(box.resolver.rootNode) : void 0,
    [box]
  );
  const root = React3.useSyncExternalStore(subscribe, getSnapshot);
  if (!root) {
    return /* @__PURE__ */ jsxRuntime.jsx(LoadingPlaceholder, { componentId: "root" });
  }
  return /* @__PURE__ */ jsxRuntime.jsx(NodeSurfaceContext.Provider, { value: surface, children: /* @__PURE__ */ jsxRuntime.jsx(NodeView, { surface, node: root }) });
};
function createComponentImplementation(api, RenderComponent) {
  ...
  const NodeView2 = ({ node, buildChild }) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx(MemoizedRender, { props: viewProps, buildChild: viewBuildChild, context });
  };
...
function createBinderlessComponentImplementation(api, RenderComponent) {
  const NodeView2 = ({ node, buildChild }) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx(RenderComponent, { context, buildChild: rawBuildChild });
  };
  NodeView2.displayName = `${api.name}.view`;
  return {
    name: api.name,
...
function MarkdownText({
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { className: classes, style, ...contentProps });
}
function NonMarkdownText({
  ...
  if (isCaption) {
    return /* @__PURE__ */ jsxRuntime.jsx("span", { className, style, children: /* @__PURE__ */ jsxRuntime.jsx(HeadingTag, { children: text }) });
  }
  return /* @__PURE__ */ jsxRuntime.jsx("div", { className, style, children: /* @__PURE__ */ jsxRuntime.jsx(HeadingTag, { children: text }) });
}
var Text = createComponentImplementation(basic_catalog.TextApi, ({ props }) => {
  ...
  if (variant && NON_MARKDOWN_VARIANTS.has(variant)) {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx(NonMarkdownText, { text, variant, className: className2, style });
  }
  const className = [Text_default.a2uiText, variant || "body"].join(" ");
  return /* @__PURE__ */ jsxRuntime.jsx(MarkdownText, { text, className, style });
});
...
var ChildList = ({ childList, buildChild }) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: childList.map((childRef, index) => {
    if (typeof childRef === "string") {
      return /* @__PURE__ */ jsxRuntime.jsx(React3__default.default.Fragment, { children: buildChild(childRef) }, `${childRef}-${index}`);
    }
    return /* @__PURE__ */ jsxRuntime.jsx(React3__default.default.Fragment, { children: buildChild(childRef.id, childRef.basePath) }, `${childRef.id}-${childRef.basePath}`);
  }) });
};
...
var Card = createComponentImplementation(basic_catalog.CardApi, ({ props, buildChild }) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "a2ui-card", style, children: props.child ? buildChild(props.child) : null });
});
...
var ChoicePicker = createComponentImplementation(basic_catalog.ChoicePickerApi, ({ props }) => {
  useBasicCatalogStyles();
  const [filter, setFilter] = React3.useState("");
  const groupName = React3__default.default.useId();
